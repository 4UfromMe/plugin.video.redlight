...
import os import threading from urllib.parse import urlparse from caches.main_cache import cache_object from modules import source_utils from modules.kodi_utils import list_dirs, open_file from modules.utils import clean_file_name, normalize, make_thread_list from modules.settings import filter_by_name, max_threads

from modules.kodi_utils import logger

class source: def init(self, scrape_provider, scraper_name, folder_path): self.scrape_provider = scrape_provider self.scraper_name = scraper_name self.folder_path = folder_path self.sources, self.scrape_results = [], [] self.extensions = source_utils.supported_video_extensions() # Track folders matched by folder-name prefiltering. # Files inside these folders bypass per-file title/alias checks. self._matched_folders = set() self._matched_folders_lock = threading.Lock()
		...
		def _process():
			for item in self.scrape_results:
				try:
					...
					if filter_title and not file_from_matched_folder and not source_utils.check_title(title, file_name, aliases, self.year, self.season, self.episode):
						continue
