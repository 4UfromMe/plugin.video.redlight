...
class source:
	def __init__(self, scrape_provider, scraper_name, folder_path):
		...
		# Track folders matched by folder-name prefiltering.
		# Files inside these folders bypass per-file title/alias checks.
	...
	def _folder_matches(self, item_name):
		# names that clean down to nothing are NOT trusted as matched
		if not item_name: return False
		if self.title_query and self.title_query in item_name: return True
		return any(x and x in item_name for x in self.folder_query)
